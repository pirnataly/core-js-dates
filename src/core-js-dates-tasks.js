...
function getCountDaysInMonth(month, year) {
  const oneDayMs = 24 * 60 * 60 * 1000;
  const date = (new Date(year, month) - new Date(year, month - 1)) / oneDayMs;
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const oneDayms = 24 * 60 * 60 * 1000;
  const date = (new Date(dateEnd) - new Date(dateStart)) / oneDayms;
  return date + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  const to = 11;
  for (let i = fromMonth; i <= to; i += 1) {
    if (new Date(fromYear, i, 13).getDay() === 5) {
      res = new Date(fromYear, i, 13);
      break;
    }
  }
  return res;
